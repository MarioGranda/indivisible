...
export const insertDao = async (input: CreateDaoInput): Promise<number> => {
...
export const findAllDaos = async (): Promise<Dao[]> => {
...
export const findDaoBySlug = async (slug: string): Promise<Dao> => {
  const query = `select d.* 
        from ${TABLE_NAMES.DAO} d
        where d.slug = ?
      `;
  ...
  const [rows] = await connection.query<RowDataPacket[]>(query, slug);
