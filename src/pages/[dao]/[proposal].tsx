...
const Proposal: FC<Props> = ({ proposal, comments }) => {
    ...
    const postClassName= "bg-gray-dark text-white text-13 p-4 rounded-xl w-full outline-none"
    ...
    return (
        <div className="explore-bg bg-cover text-white font-source min-h-screen">
            <FixedContainer className="flex flex-col justify-items-center gap-5">
                <button className="flex gap-5 items-center rounded-xl bg-gray-dark mt-10 p-4 w-[270px]"
                    onClick={() => setIsNewComment(!isNewComment)}>
                    <MdOutlineAdd size={40} />Add new comment
                </button>
                {isNewComment &&
