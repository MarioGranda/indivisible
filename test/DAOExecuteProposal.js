...
describe("DAO executeProposal tests", async function () {
    ...
    it("Success: Executes proposal", async function () {
        ...
        const tx = await dao.connect(signers[3]).newProposal(
            consensusPeriod,
            votingPeriod,
            minQuorum,
            tokenAddress,
    ...
    it("Fails: Non-creator proposal execute it", async function () {
        ...
        //Proposal creator executes proposal
        await expect(dao.connect(signers[4]).executeProposal(
            proposalID,
            { from: voter })).to.be.revertedWith('DAO: Only proposal creator can execute this proposal')
    });
    ...
    it("Fails: Executes proposal before voting deadline", async function () {
        ...
        await expect(dao.connect(signers[3]).executeProposal(
            proposalID,
            { from: user })).to.be.revertedWith('DAO: Voting has not finished yet')
    });
    ...
    it("Fails: Min Quorum proposal has not been achieved", async function () {
        ...
        await expect(dao.connect(signers[3]).executeProposal(
            proposalID,
            { from: user })).to.be.revertedWith("DAO: Quorum must be greater than proposal's minQuorum")
    });
