...
describe("DAO withdrawFunds tests", async function () {
    let signers, user, multisig, daoDeployer, daoCreator, dao, daoAddress, tokenAddress, voter, token;
    let DAO, Token;
    ...
    beforeEach(async function () {
        ...
        //create DAO
        let tx = await
            daoCreator.connect(signers[1]).createDAO(name,
                symbol,
                mintAmount,
                minConsensusPeriod,
                minVotingPeriod,
        ...
        tx = await dao.connect(signers[3]).newProposal(
            consensusPeriod,
            votingPeriod,
            minQuorum,
            tokenAddress,
        ...
        //Vote on proposal
        await dao.connect(signers[4]).vote(
            proposalID,
            vote,
            depositAmount,
            lockingSeconds,
    ...
    it("Success: Withdraw funds after executing proposal", async function () {
        ...
        await expect(dao.connect(signers[3]).executeProposal(
            proposalID,
            { from: user })).to.emit(dao, "ProposalExecuted").withArgs(proposalID, true)

        //Voter withdraws funds
        await expect(dao.connect(signers[4]).getMyFunds(
            proposalID,
            { from: voter })).to.emit(token, "Transfer").withArgs(daoAddress, voter, depositAmount)
    });
    ...
    it("Fails: Withdraw funds before executing proposal", async function () {
        ...
        await expect(dao.connect(signers[4]).getMyFunds(
            proposalID,
            { from: voter })).to.be.revertedWith('DAO: Proposal has not been closed yet')
    });
    ...
    it("Fails: Withdraw funds without voting", async function () {
        ...
        //Voter withdraws funds
        await expect(dao.connect(signers[3]).getMyFunds(
            proposalID,
            { from: user })).to.be.revertedWith('DAO: No deposit was made for this proposal')
    });
});
