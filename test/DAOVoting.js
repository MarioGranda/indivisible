...
describe("DAO vote tests", async function () {
    ...
    it("Success: Voting after proposal", async function () {
        ...
        await expect(dao.connect(signers[4]).vote(
            proposalID,
            vote,
            depositAmount,
            lockingSeconds,
            tokenAddress,
            [],
            { from: voter })).to.emit(dao, "Voted").withArgs(proposalID, vote, voter)
    });
    ...
    it("Fails: Voting before consensus period finishes", async function () {
        ...
        await expect(dao.connect(signers[4]).vote(
            ...
            tokenAddress,
            [],
            { from: voter })).to.be.revertedWith('DAO: Voting has not started yet')
    });
    ...
    it("Fails: Voting after voting deadline", async function () {
        ...
        await expect(dao.connect(signers[4]).vote(
            ...
            tokenAddress,
            [],
            { from: voter })).to.be.revertedWith('DAO: Voting has finished')
    });
    ...
    it("Fails: Voter cannot vote more than 1 time", async function () {
        ...
        await expect(dao.connect(signers[4]).vote(
            ...
            tokenAddress,
            [],
            { from: voter })).to.be.revertedWith('DAO: User has already voted')
    });
});
