...
const { deployContracts, generateMerkleRoot, generateMerkleProof, verifyMerkleProof, generateMerkleTree } = require("./helpers");

describe("DAO coalition tests", async function () {
    let signers, user, multisig, daoDeployer, dao1Address, dao2Address, daoCreator, token1Address, coalitionDao;
    ...
    beforeEach(async function () {
        ...
        await coalitionDao.connect(signers[1]).updateMerkleRoot(merkleRoot, { from: daoDeployer })
    });
    it("Success: Create coalition DAO & submit proposal from subDao", async function () {
        ...
        await expect(coalitionDao.connect(signers[3]).newProposal(
            minConsensusPeriod,
            minVotingPeriod,
            minQuorum,
            token1Address,
            merkleProof,
            { from: user })).to.emit(coalitionDao, "ProposalAdded")
    });
    ...
    it("Fails: Create coalition DAO & submit proposal from fake subDao", async function () {
        ...
        await expect(coalitionDao.connect(signers[3]).newProposal(
            ...
            fakeTokenAddress,
            merkleProof,
            { from: user })).to.be.revertedWith('DAO: Token does not belong to any subDao')
    });
